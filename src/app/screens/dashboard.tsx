...
export function Dashboard() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (user) {
        ...
        await checkAndCreateProfile(user.id, user.user_metadata);
      }
      setAuthChecked(true);
    };
    getUser();
  ...
  const checkAndCreateProfile = async (userId: string, metadata: any) => {
    ...
    if (!existingProfile) {
      await supabase
        ...
        .insert({
          id: userId,
          name: metadata?.full_name || 'Creator',
